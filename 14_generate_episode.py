...
def main() -> None:
    ...
    mark_step_started("14_generate_episode", autosave_target, {"series_model": str(model_path)})
    if shared_workers:
        info(f"Shared NAS workers: enabled ({worker_id})")
    lease_manager = distributed_item_lease(
        root=distributed_step_runtime_root("14_generate_episode", autosave_target),
        lease_name=autosave_target,
        cfg=cfg,
        worker_id=worker_id,
        enabled=shared_workers,
        meta={"step": "14_generate_episode", "scope": autosave_target, "worker_id": worker_id},
    )
    ...
    reporter = LiveProgressReporter(
        script_name="14_generate_episode.py",
        total=5,
