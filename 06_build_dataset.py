...
def main() -> None:
    ...
    lease_root = distributed_step_runtime_root("06_build_dataset", "episodes")
    if shared_workers:
        info(f"Shared NAS workers: enabled ({worker_id})")
    live_reporter = LiveProgressReporter(
        script_name="06_build_dataset.py",
        total=max(1, total),
    ...
    for index, episode_dir in enumerate(episode_dirs, start=1):
        with distributed_item_lease(
            ...
            meta={"step": "06_build_dataset", "episode_id": episode_dir.name, "worker_id": worker_id},
        ) as acquired:
            if not acquired:
                continue
