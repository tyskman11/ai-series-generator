...
def run_step(
    ...
    result = subprocess.run(
        [str(runtime_python()), str(WORKSPACE_ROOT / script_name), *(extra_args or []), *(shared_args or [])],
        env=env,
        cwd=str(SCRIPT_DIR),
    )
...
def training_plan_rows(cfg: dict, skip_downloads: bool = False) -> list[tuple[str, str, list[str]]]:
    ...
    rows: list[tuple[str, str, list[str]]] = [
        ("06_manage_character_relationships.py", "Manage Character Groups", []),
...
def planned_preview_steps_for_mode(cfg: dict, count: int, *, endless: bool) -> list[str]:
    ...
    if release_mode_enabled(cfg):
        generation_cycle.append("18_quality_gate.py")
    if endless:
        steps.extend(generation_cycle)
        steps.append("20_build_series_bible.py")
        steps.append("21_export_package.py")
        return steps
...
def main() -> None:
    ...
    mark_step_started(
        "23_generate_finished_episodes",
        autosave_target,
        {"requested_count": count, "endless": endless, "skip_downloads": bool(args.skip_downloads)},
    )
    lease_manager = distributed_item_lease(
        root=distributed_step_runtime_root("23_generate_finished_episodes", autosave_target),
        lease_name=autosave_target,
        cfg=cfg,
        worker_id=worker_id,
        enabled=shared_workers,
        meta={"step": "23_generate_finished_episodes", "scope": autosave_target, "worker_id": worker_id},
    )
    ...
    try:
        ...
        cfg = load_config()
        ensure_quality_first_ready(cfg, context_label="23_generate_finished_episodes.py")
        ...
        reporter = LiveProgressReporter(
            script_name="23_generate_finished_episodes.py",
        ...
        while endless or index < count:
            ...
            reporter.update(
                ...
                extra_label="Running now: 14_generate_episode.py",
                force=True,
                scope_current=len(generated),
                scope_total=preview_scope_total(count, endless, len(generated)),
                scope_label="Episodes",
            )
            run_step("14_generate_episode.py", shared_args=child_shared_args)
            completed_steps += 1
            ...
            run_step("16_run_storyboard_backend.py", env=env, extra_args=storyboard_backend_args, shared_args=child_shared_args)
            completed_steps += 1
            reporter.update(
                preview_reporter_current(reporter, completed_steps),
                current_label=episode_label,
            ...
            run_step("17_render_episode.py", env=env, extra_args=render_args, shared_args=child_shared_args)
            completed_steps += 1
            run_generation_toolkit_phase(cfg, "post_render", episode_id)
            reporter.update(
                preview_reporter_current(reporter, completed_steps),
                current_label=f"{episode_label} quality gate",
                extra_label="Running now: 18_quality_gate.py",
                force=True,
                scope_current=len(generated) + 1,
                scope_total=preview_scope_total(count, endless, len(generated)),
                scope_label="Episodes",
            ...
            if endless:
                reporter.update(
                    ...
                    extra_label="Running now: 20_build_series_bible.py",
                    force=True,
                    scope_current=len(generated),
                    scope_total=preview_scope_total(count, endless, len(generated)),
                    scope_label="Episodes",
                )
                run_step("20_build_series_bible.py", shared_args=child_shared_args)
                completed_steps += 1
                reporter.update(
                    preview_reporter_current(reporter, completed_steps),
                    current_label="Export episode package",
                    extra_label="Running now: 21_export_package.py",
                    force=True,
                    scope_current=len(generated),
                    scope_total=preview_scope_total(count, endless, len(generated)),
                    scope_label="Episodes",
                )
                run_step("21_export_package.py", shared_args=child_shared_args)
                completed_steps += 1
                run_generation_toolkit_phase(cfg, "post_export", episode_id)
                reporter.update(
                    preview_reporter_current(reporter, completed_steps),
                    current_label="Series bible and export updated",
        ...
        if not endless:
            ...
            reporter.update(
                completed_steps,
                current_label="Update series bible",
                extra_label="Completed: 20_build_series_bible.py",
            )
            reporter.update(
                completed_steps,
                current_label="Export episode package",
            ...
            run_step("21_export_package.py", shared_args=child_shared_args)
            completed_steps += 1
            latest_exported_episode = generated[-1] if generated else None
            run_generation_toolkit_phase(cfg, "post_export", latest_exported_episode)
            reporter.update(
                completed_steps,
                current_label="Export episode package",
                extra_label="Completed: 21_export_package.py",
            )
        ...
        mark_step_completed(
            ...
            autosave_target,
            {
                ...
                "package_exported_once_after_batch": not endless,
                "package_exported_after_each_episode": endless,
            },
        )
        ...
        if generated and endless:
            ...
            run_step("20_build_series_bible.py", shared_args=child_shared_args)
            run_step("21_export_package.py", shared_args=child_shared_args)
            run_generation_toolkit_phase(cfg, "post_export", generated[-1])
        mark_step_failed(
            "23_generate_finished_episodes",
            "Interrupted by user",
            autosave_target,
