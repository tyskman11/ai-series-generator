...
GENERATED_BACKEND_MODES = {
    "configured_external_storyboard_runner",
    "external_storyboard_runner",
}

GENERATED_BACKEND_SOURCE_TYPES = {
    "configured_external_runner",
    "external_backend_runner",
    "external_generated_frame",
}


def storyboard_backend_settings(cfg: dict | None) -> dict:
...
def allow_existing_local_frame_reuse(cfg: dict | None) -> bool:
    settings = storyboard_backend_settings(cfg)
    return bool(settings.get("allow_existing_local_frame_reuse", False))


def backend_manifest_is_external_generated(manifest: dict | None) -> bool:
    if not isinstance(manifest, dict):
        return False
    backend_mode = str(manifest.get("backend_mode", "") or "").strip()
    source_type = str(manifest.get("source_type", "") or "").strip()
    return backend_mode in GENERATED_BACKEND_MODES or source_type in GENERATED_BACKEND_SOURCE_TYPES


def storyboard_backend_manifest_mode(rows: list[dict]) -> str:
    if not rows:
        return "missing_storyboard_backend"
    generated_count = sum(1 for row in rows if backend_manifest_is_external_generated(row))
    if generated_count >= len(rows):
        return "configured_external_storyboard_runner"
    if generated_count > 0:
        return "mixed_storyboard_backend"
    return "materialized_local_backend_scene_pack"


def remove_stale_backend_outputs(paths: list[Path]) -> None:
    for path in paths:
        try:
    ...
    try:
        ...
        write_json(
            manifest_path,
            {
                ...
                "backend_mode": manifest_backend_mode,
                "scene_runs": rows,
        ...
        shotlist["storyboard_backend_mode"] = manifest_backend_mode
