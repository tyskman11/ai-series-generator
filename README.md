...
- the project-local Transformers screenwriter accepts both tensor and `BatchEncoding` chat-template outputs, passes `input_ids` and optional attention masks correctly, retries local JSON/speaker failures up to `local_generation.scriptwriter.max_generation_attempts` (default `3`), and preserves complete locally generated dialogue rows when one later JSON row is malformed; it emits a clear warning instead of silently using a template fallback
...
- `local_generation.scriptwriter.*`: local Qwen dialogue model settings. It must remain `enabled`, `engine="transformers"`, and `local_files_only=true` in Quality-First mode. `max_generation_attempts` controls local format/speaker retries; when all attempts fail, the raw local outputs are saved under `logs/screenwriter/` for diagnosis instead of using a template fallback
