...
- `62_trend_analyzer.py`: analyze series trends
- `63_review_queue.py`: review workflow queue system
- `64_scene_transition_selector.py`: select optimal scene transitions
- `65_weather_detector.py`: detect weather conditions in scenes
- `66_episode_archive.py`: archive old episodes with retention
- `67_training_optimizer.py`: optimize training parameters
- `68_voice_emotion_cloning.py`: clone voice with emotion
- `69_social_media_clips.py`: generate social media clips (TikTok, Reels, Shorts)

### Post-Process Pipeline (70-77)

- `70_refresh_after_manual_review.py`: rebuild the pipeline after manual character review
- `71_run_backend_finetunes.py`: materialize backend-oriented fine-tune runs
- `72_export_package.py`: export finished episode package for external tools
- `73_quality_gate.py`: automatic quality check before release
- `74_compare_scenes.py`: compare two scenes visually for QA
- `75_run_storyboard_backend.py`: materialize local backend-style storyboard scene packs
- `76_backup_project.py`: backup project to cloud/NAS
- `77_restore_project.py`: restore project from backup
