...
def planned_refresh_steps(cfg: dict, skip_downloads: bool = False, stop_after_training: bool = False) -> list[tuple[str, str, list[str]]]:
    ...
    planned_steps: list[tuple[str, str, list[str]]] = [
        ("06_build_dataset.py", "Rebuild datasets with the latest reviewed character names", ["--force"]),
        ("07_train_series_model.py", "Retrain the series model with the latest reviewed names", []),
    ]
    prepare_args = ["--force"]
    if skip_downloads:
    ...
    if bool(foundation_cfg.get("prepare_after_batch", False)) or needs_foundation_training:
        planned_steps.append(
            ("08_prepare_foundation_training.py", "Prepare foundation training with the latest reviewed character state", prepare_args)
        )
        if bool(foundation_cfg.get("auto_train_after_prepare", False)) or needs_foundation_training:
            planned_steps.append(
                ("09_train_foundation_models.py", "Retrain foundation packs with the latest reviewed character state", ["--force"])
            )
            if bool(adapter_cfg.get("auto_train_after_foundation", True)):
                planned_steps.append(
                    ("10_train_adapter_models.py", "Retrain local adapter profiles with the latest reviewed character state", ["--force"])
                )
                if bool(fine_tune_cfg.get("auto_train_after_adapter", True)):
                    planned_steps.append(
                        ("11_train_fine_tune_models.py", "Retrain local fine-tune profiles with the latest reviewed character state", ["--force"])
                    )
...
def main() -> None:
    ...
    if not args.stop_after_training:
        ensure_quality_first_ready(cfg, context_label="48_refresh_after_manual_review.py")
    autosave_target = "global"
    if shared_workers:
        info(f"Shared NAS workers: enabled ({worker_id})")
    mark_step_started(
        "48_refresh_after_manual_review",
        autosave_target,
        {
    ...
    lease_manager = distributed_item_lease(
        root=distributed_step_runtime_root("48_refresh_after_manual_review", autosave_target),
        lease_name=autosave_target,
        cfg=cfg,
        worker_id=worker_id,
        enabled=shared_workers,
        meta={"step": "48_refresh_after_manual_review", "scope": autosave_target, "worker_id": worker_id},
    )
    ...
    try:
        ...
        reporter = LiveProgressReporter(
            script_name="48_refresh_after_manual_review.py",
