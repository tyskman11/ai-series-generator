...
from support_scripts.pipeline_common import (
    ...
    check_character_continuity_violations,
    detect_tool,
...
STEP05 = load_module("04_link_faces_and_speakers.py", "step05")
...
class ManualNamingTests(unittest.TestCase):
    def test_pip_install_command_adds_break_system_packages_when_supported(self) -> None:
        with mock.patch("support_scripts.pipeline_common.pip_supports_break_system_packages", return_value=True):
    ...
    def test_pip_install_command_skips_break_system_packages_when_unsupported(self) -> None:
        with mock.patch("support_scripts.pipeline_common.pip_supports_break_system_packages", return_value=False):
            command = pip_install_command("C:/Python/python.exe", "--upgrade", "wheel")
        self.assertEqual(
            command,
    ...
    def test_distributed_lease_allows_takeover_after_expiry(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("support_scripts.pipeline_common.time.time", return_value=100.0):
                first = acquire_distributed_lease(lease_root, "scene_001", "worker-a", 30.0, meta={"scene_id": "scene_001"})
            self.assertIsNotNone(first)
            with mock.patch("support_scripts.pipeline_common.time.time", return_value=105.0):
                second = acquire_distributed_lease(lease_root, "scene_001", "worker-b", 30.0)
            self.assertIsNone(second)
            with mock.patch("support_scripts.pipeline_common.time.time", return_value=150.0):
    ...
    def test_step04_parse_args_supports_shared_worker_flags(self) -> None:
        with mock.patch(
            "sys.argv",
            ["03_diarize_and_transcribe.py", "--episode", "Demo.S01E01", "--no-shared-workers", "--worker-id", "pc2"],
        ):
    ...
    def test_detect_tool_ignores_windows_exe_on_linux(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("support_scripts.pipeline_common.current_os", return_value="linux"), mock.patch(
                "support_scripts.pipeline_common.tool_on_path", return_value=None
            ):
    ...
    def test_detect_tool_prefers_linux_binary_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("support_scripts.pipeline_common.current_os", return_value="linux"):
                self.assertEqual(detect_tool(bin_dir, "ffmpeg"), linux_binary)

    def test_detect_tool_uses_python_packaged_ffmpeg_when_local_binary_is_missing(self) -> None:
        packaged = Path(tempfile.gettempdir()) / "packaged_ffmpeg.exe"
        project_like_dir = Path(tempfile.mkdtemp()) / "ai_series_project" / "tools" / "ffmpeg" / "bin"
        project_like_dir.mkdir(parents=True, exist_ok=True)
        with mock.patch("support_scripts.pipeline_common.python_packaged_tool", return_value=packaged), mock.patch(
            "support_scripts.pipeline_common.prefer_python_packaged_tool", return_value=True
        ), mock.patch("support_scripts.pipeline_common.tool_on_path", return_value=None):
    ...
    def test_runtime_python_uses_active_python_on_linux(self) -> None:
        with mock.patch("support_scripts.pipeline_common.current_os", return_value="linux"), mock.patch("support_scripts.pipeline_common.sys.executable", "/usr/bin/python3"):
    ...
    def test_resolve_stored_project_path_rebases_old_ai_series_project_path(self) -> None:
        rebased_target = ROOT / "tmp" / "rebased_demo.json"
    ...
    def test_external_tool_arg_strips_windows_extended_unc_prefix(self) -> None:
        with mock.patch("support_scripts.pipeline_common.current_os", return_value="windows"):
    ...
    def test_auto_learn_remaining_reviews_iterates_until_no_face_matches_remain(self) -> None:
        cfg = {
            "paths": {"linked_segments": str(ROOT / "data" / "processed" / "linked_segments")},
            "character_detection": {
    ...
    def test_parse_args_defaults_to_twenty_and_supports_all(self) -> None:
        ...
        with mock.patch("sys.argv", ["05_review_unknowns.py", "--all"]):
            args_all = STEP08.parse_args()
        self.assertTrue(args_all.all)

        with mock.patch("sys.argv", ["05_review_unknowns.py", "--auto-mark-statists", "--statist-max-scenes", "1"]):
    ...
    def test_build_original_line_library_uses_linked_speaker_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("support_scripts.pipeline_common.PROJECT_ROOT", root):
    ...
    def test_build_scene_voice_plan_scales_to_scene_duration_and_reuses_sources(self) -> None:
        ...
        self.assertGreater(plan[-1]["end_seconds"], 7.2)
        self.assertLessEqual(plan[-1]["end_seconds"], 8.4)
    ...
    def test_render_subtitle_preview_srt_formats_dialogue_entries(self) -> None:
        ...
        self.assertIn("00:00:00,000 --> 00:00:01,600", srt)
    ...
    def test_materialize_scene_backend_frame_ignores_directory_source_placeholders(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            assets_root = root / "assets" / "episode_011"
            assets_root.mkdir(parents=True, exist_ok=True)
            ...
            self.assertEqual(manifest["source_type"], "missing")
    ...
    def test_normalized_scene_dialogue_outputs_tolerates_missing_key(self) -> None:
        self.assertEqual(STEPRENDER.normalized_scene_dialogue_outputs({}), {})
        self.assertEqual(STEPRENDER.normalized_scene_dialogue_outputs({"scene_dialogue_outputs": None}), {})
        self.assertEqual(
            STEPRENDER.normalized_scene_dialogue_outputs(
                {"scene_dialogue_outputs": {"scene_0001": " C:/demo/scene_0001.wav ", "": "ignored"}}
            ),
            {"scene_0001": "C:/demo/scene_0001.wav"},
        )
    ...
    def test_foundation_training_status_detects_outdated_training(self) -> None:
        with tempfile.TemporaryDirectory(dir=str(ROOT / "tmp"), ignore_cleanup_errors=True) as tmp:
            temp_root = Path(tmp)
            checkpoints_rel = Path("tmp") / temp_root.name / "checkpoints"
            model_rel = Path("tmp") / temp_root.name / "series_model.json"
            checkpoints_dir = ROOT / checkpoints_rel
    ...
    def test_adapter_training_status_detects_outdated_training(self) -> None:
        with tempfile.TemporaryDirectory(dir=str(ROOT / "tmp"), ignore_cleanup_errors=True) as tmp:
            ...
            checkpoint_dir = ROOT / checkpoints_rel
    ...
    def test_preview_main_blocks_only_on_face_review_cases(self) -> None:
        ...
        with mock.patch.object(STEP19PREVIEW, "rerun_in_runtime"), mock.patch.object(
            ...
            return_value={
                ...
                "delivery_manifest": "C:\\demo\\episode_001_delivery_manifest.json",
                "delivery_episode": "C:\\demo\\episode_001_delivery_episode.mp4",
    ...
    def test_fine_tune_training_status_detects_outdated_training(self) -> None:
        with tempfile.TemporaryDirectory(dir=str(ROOT / "tmp")) as tmp:
            ...
            adapter_dir = ROOT / adapter_rel
            finetune_dir = ROOT / finetune_rel
            adapter_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_check_character_continuity_violations_detects_attribute_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Seed continuity memory with a known outfit
            memory = {
                "characters": {
                    "Alice": {
                        "appearances": {},
                        "continuity": {"outfit": "blue_dress"},
                    }
                },
                "last_updated": None,
            }
            mem_path = project_root / "characters" / "continuity_memory.json"
            mem_path.parent.mkdir(parents=True, exist_ok=True)
            mem_path.write_text(json.dumps(memory), encoding="utf-8")

            # Current state has a different outfit
            current_states = {
                "Alice": {"outfit": "red_dress"},
            }

            violations = check_character_continuity_violations(
                project_root, current_states
            )

            self.assertEqual(len(violations), 1)
            self.assertEqual(violations[0]["character"], "Alice")
            self.assertEqual(violations[0]["type"], "attribute_change")
            self.assertEqual(violations[0]["attribute"], "outfit")
            self.assertEqual(violations[0]["previous"], "blue_dress")
            self.assertEqual(violations[0]["current"], "red_dress")

    def test_check_character_continuity_violations_no_violation_when_matching(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            memory = {
                "characters": {
