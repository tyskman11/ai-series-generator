...
def write_scene_backend_input(
    ...
    payload = {
        ...
        "style_constraints": generation_plan.get("style_constraints", {}),
        "character_continuity": generation_plan.get("character_continuity", []),
        "quality_targets": generation_plan.get("quality_targets", {}),
...
def main() -> None:
    ...
    if not episode_request_path.exists():
        info("No storyboard request export found. Run 12_generate_episode.py first.")
        return
    ...
    mark_step_started("13_generate_storyboard_assets", autosave_target, {"episode_id": episode_id, "shotlist": str(shotlist_path)})
    reporter = LiveProgressReporter(
        script_name="13_generate_storyboard_assets.py",
        total=max(1, len(scene_requests)),
        phase_label="Generate Storyboard Assets",
        parent_label=episode_id,
    )
    scene_lease_root = distributed_step_runtime_root("13_generate_storyboard_assets", episode_id) / "scenes"
    if shared_workers:
    ...
    try:
        ...
        for index, scene_request in enumerate(scene_requests, start=1):
            ...
            with distributed_item_lease(
                ...
                enabled=shared_workers,
                        meta={"step": "13_generate_storyboard_assets", "episode_id": episode_id, "scene_id": scene_id, "worker_id": worker_id},
            ) as acquired:
                if not acquired:
                    continue
        ...
        mark_step_failed("13_generate_storyboard_assets", str(exc), autosave_target, {"episode_id": episode_id})
        raise
