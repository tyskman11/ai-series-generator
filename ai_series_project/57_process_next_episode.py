...
def autosave_dir(cfg: dict) -> Path:
    return resolve_project_path("runtime/autosaves/57_process_next_episode")
...
def episode_step_title(script_name: str, episode_file: str, episode_name: str) -> str:
    titles = {
        ...
        "07_train_series_model.py": "Train series model from reviewed data",
    }
...
def global_step_rows(cfg: dict, skip_downloads: bool = False) -> list[tuple[str, str, list[str]]]:
    ...
    if bool(foundation_cfg.get("prepare_after_batch", False)) or needs_foundation_training:
        ...
        steps.append(("08_prepare_foundation_training.py", "Prepare Foundation Training", prepare_args))
        if bool(foundation_cfg.get("auto_train_after_prepare", False)) or needs_foundation_training:
            steps.append(("09_train_foundation_models.py", "Train Foundation Models", []))
            if bool(adapter_cfg.get("auto_train_after_foundation", True)):
                steps.append(("10_train_adapter_models.py", "Train Local Adapter Profiles", []))
...
def global_step_title(script_name: str) -> str:
    titles = {
        "06_build_dataset.py": "Build training dataset from reviewed data",
        "07_train_series_model.py": "Train Series Model",
        "08_prepare_foundation_training.py": "Prepare Foundation Training",
...
def main() -> None:
    ...
    lease_manager = distributed_item_lease(
        root=distributed_step_runtime_root("57_process_next_episode", "global"),
        lease_name="global",
        cfg=cfg,
        worker_id=worker_id,
        enabled=shared_workers,
        meta={"step": "57_process_next_episode", "scope": "global", "worker_id": worker_id},
    )
    ...
    try:
        ...
        episode_batch_reporter = LiveProgressReporter(
            script_name="57_process_next_episode.py",
        ...
        cfg = load_config()
        ensure_quality_first_ready(cfg, context_label="57_process_next_episode.py")
