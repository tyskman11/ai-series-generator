...
def main() -> None:
    ...
    mark_step_started("08_prepare_foundation_training", autosave_target, {"episode_filter": coalesce_text(args.episode or "")})
    foundation_cfg = cfg.get("foundation_training", {}) if isinstance(cfg.get("foundation_training"), dict) else {}
    ...
    try:
        if not candidates:
            ...
            mark_step_completed(
                "08_prepare_foundation_training",
                autosave_target,
        ...
        lease_root = distributed_step_runtime_root("08_prepare_foundation_training", autosave_target)
        reporter = LiveProgressReporter(
            script_name="08_prepare_foundation_training.py",
        ...
        for index, character in enumerate(candidates, start=1):
            ...
            with distributed_item_lease(
                ...
                meta={"step": "08_prepare_foundation_training", "character": character["name"], "slug": character["slug"], "worker_id": worker_id},
            ) as acquired:
                if not acquired:
                    continue
        ...
        mark_step_failed("08_prepare_foundation_training", str(exc), autosave_target)
        raise
