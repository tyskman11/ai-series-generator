...
def main() -> None:
    ...
    lease_root = distributed_step_runtime_root("02_split_scenes", "episodes")
    if shared_workers:
    ...
    if args.episode_file:
        ...
        with distributed_item_lease(
            ...
            meta={"step": "02_split_scenes", "episode_id": episode.stem, "worker_id": worker_id},
        ) as acquired:
            if not acquired:
    ...
    live_reporter = LiveProgressReporter(
        script_name="02_split_scenes.py",
    ...
    for index, current_episode in enumerate(pending, start=1):
        with distributed_item_lease(
            ...
            worker_id=worker_id,
            enabled=shared_workers,
            meta={"step": "02_split_scenes", "episode_id": current_episode.stem, "worker_id": worker_id},
        ) as acquired:
            if not acquired:
                continue
