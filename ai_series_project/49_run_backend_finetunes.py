        ...
        rows.append(
            {
                ...
                "autosave": load_step_autosave("49_run_backend_finetunes", coalesce_text(payload.get("slug", ""))),
            }
        )
...
def main() -> None:
    ...
    if not fine_rows:
        info("No fine-tune summary found. Run 11_train_fine_tune_models.py first.")
        return
    ...
    lease_root = distributed_step_runtime_root("49_run_backend_finetunes", "characters")
    reporter = LiveProgressReporter(
        script_name="49_run_backend_finetunes.py",
    ...
    for index, row in enumerate(filtered_rows, start=1):
        ...
        with distributed_item_lease(
            ...
            enabled=shared_workers,
                    meta={"step": "49_run_backend_finetunes", "character": character_name, "slug": slug, "worker_id": worker_id},
        ) as acquired:
            if not acquired:
                continue
