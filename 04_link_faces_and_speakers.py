...
def episode_face_linking_completed(episode_dir: Path, cfg: dict) -> bool:
    ...
    autosave_state = completed_step_state("04_link_faces_and_speakers", episode_dir.name, PROCESS_VERSION)
    if marker:
    ...
    *,
    ...
    try:
        ...
        link_reporter = LiveProgressReporter(
            script_name="04_link_faces_and_speakers.py",
...
def main() -> None:
    ...
    lease_root = distributed_step_runtime_root("04_link_faces_and_speakers", "episodes")
    ...
    for index, episode_dir in enumerate(episode_dirs, start=1):
        with distributed_item_lease(
            ...
            meta={"step": "04_link_faces_and_speakers", "episode_id": episode_dir.name, "worker_id": worker_id},
        ) as acquired:
            if not acquired:
                continue
