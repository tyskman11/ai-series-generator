...
from support_scripts.generation_toolkit import run_generation_toolkit_phase
...
def run_step(
    ...
    if not script_path.exists():
        raise FileNotFoundError(f"Required pipeline script is missing: {script_path}")
    command = [str(runtime_python()), str(script_path), *(extra_args or []), *(shared_args or [])]
    result = subprocess.run(command, cwd=str(WORKSPACE_ROOT))
...
def autosave_dir(cfg: dict) -> Path:
    return resolve_project_path("runtime/autosaves/24_process_next_episode")
...
def setup_refresh_reason(cfg: dict, state: dict) -> str:
    if not bool(state.get("setup_completed", False)):
        return "setup has not completed yet"
    report = quality_first_requirements_report(cfg)
    missing = [str(entry).strip() for entry in report.get("missing", []) if str(entry).strip()]
    refreshable = [
        entry
        for entry in missing
        if entry.startswith("external_backends.")
        and (
            "uses fallback backend" in entry
            or "requires environment variable" in entry
            or "requires Python module" in entry
            or "must be enabled" in entry
        )
    ]
    if refreshable:
...
def episode_step_args(script_name: str, episode_file: str, episode_name: str) -> list[str]:
    if script_name == "01_import_episode.py":
        return ["--episode-file", episode_file]
    if script_name == "02_split_scenes.py":
        return ["--episode-file", episode_file]
    if script_name in {"03_diarize_and_transcribe.py", "04_link_faces_and_speakers.py"}:
...
def episode_step_title(script_name: str, episode_file: str, episode_name: str) -> str:
    titles = {
        "01_import_episode.py": f"Import: {episode_file}",
        "02_split_scenes.py": f"Scene Detection: {episode_name}",
        "03_diarize_and_transcribe.py": f"Audio Segmentation And Transcription: {episode_name}",
        "04_link_faces_and_speakers.py": f"Link Faces And Voices: {episode_name}",
...
def global_step_rows(cfg: dict, skip_downloads: bool = False) -> list[tuple[str, str, list[str]]]:
    ...
    steps.extend(
        [
            ("14_generate_episode.py", "Generate New Episode From Trained Model", []),
    ...
    if release_mode_enabled(cfg):
        ...
        steps.append(("18_quality_gate.py", "Run Release-Style Quality Gate", quality_gate_args))
    steps.append(("20_build_series_bible.py", "Update Series Bible", []))
    steps.append(("21_export_package.py", "Export Finished Episode Package", []))
    return steps
...
def global_step_title(script_name: str) -> str:
    titles = {
        ...
        "16_run_storyboard_backend.py": "Materialize Storyboard Backend Frames",
        "17_render_episode.py": "Render Finished Episode",
        "18_quality_gate.py": "Run Release-Style Quality Gate",
        "20_build_series_bible.py": "Update Series Bible",
        "21_export_package.py": "Export Finished Episode Package",
    }
...
def latest_episode_id_for_toolkit(cfg: dict) -> str | None:
    outputs = latest_generated_episode_artifacts(cfg)
    episode_id = str(outputs.get("episode_id", "") or "").strip() if isinstance(outputs, dict) else ""
    return episode_id or None


def run_toolkit_after_global_step(cfg: dict, script_name: str) -> None:
    episode_id = latest_episode_id_for_toolkit(cfg)
    if script_name == "14_generate_episode.py":
        run_generation_toolkit_phase(cfg, "post_story", episode_id)
    elif script_name == "17_render_episode.py":
        run_generation_toolkit_phase(cfg, "post_render", episode_id)
    elif script_name == "18_quality_gate.py":
        run_generation_toolkit_phase(cfg, "post_quality_gate", episode_id)
    elif script_name == "21_export_package.py":
...
def main() -> None:
    ...
    lease_manager = distributed_item_lease(
        ...
        meta={"step": "24_process_next_episode", "scope": "global", "worker_id": worker_id},
    )
    ...
    try:
        ...
        refresh_reason = setup_refresh_reason(cfg, state)
        if refresh_reason and bool(state.get("setup_completed", False)):
            warn(f"Setup will be refreshed before resume because the current runtime/config is stale: {refresh_reason}")
            state["setup_completed"] = False
            save_autosave(cfg, state, "setup_refresh_required", inbox_dir)
        ...
        if not bool(state.get("setup_completed", False)):
            ...
            run_step(SETUP_STEP, "Prepare Runtime And Project Setup", setup_args, shared_args=child_shared_args)
        ...
        if review_count > 0:
            info(
                ...
                "Run 05_review_unknowns.py first before dataset rebuild, training, generation, or render can continue."
            )
            state["current_phase"] = "review"
            state["current_step"] = "05_review_unknowns.py"
        ...
        state["current_step"] = None
        run_generation_toolkit_phase(cfg, "pre_training")
        ...
        for script_name, step_title, step_args in step_rows:
            if script_name in completed_global_steps:
                continue
            if script_name == "14_generate_episode.py":
                run_generation_toolkit_phase(cfg, "pre_generation")
            ...
            run_toolkit_after_global_step(cfg, script_name)
            global_reporter.update(
                len(completed_global_steps),
                current_label=step_title,
                extra_label=f"Completed: {script_name}",
